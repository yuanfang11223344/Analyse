...
3. 英文材料必须输出中文解读；每页至少包含原文摘录、图中内容理解、原文逐项解读、关键概念拆解、我的理解、对我们的启示和本页小结。
4. 禁止只写泛泛页级摘要。能抽取到原文的页面，必须逐句或逐项解释原文、命令、参数、公式和报告字段；图示页必须理解图中内容本身：它代表的电路结构、代码含义、处理流程、对象关系、箭头、边界、时间预算或层次结构。
5. 不使用多页拼接图替代逐页分析；拼接图只可作为辅助预览，不作为主要报告。
6. 更新内容目录后，同步更新该目录内的 `pages/`、`assets/`、`sources/`、索引和总览。
7. 同步 Obsidian 镜像，保持本地仓库和笔记库内容一致。
8. 目录结构、图片目录或合并版笔记发生变化后，必须校验所有 Markdown 图片链接能按相对路径解析到真实文件。
9. 提交前展示 `git status` 和变更摘要；收到确认后再 commit 和 push。
